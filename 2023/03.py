def get_char_by(line_idx, idx, list_strs):
    # 0, 0, [[1,2],[3,4]] -> 1
    char = "."
    if (
        line_idx >= 0
...
def digit_around_ids(line_idx, idx, list_strs):
    # 0, 0, [[*,.],[1,.]] -> (1, 0)
    candidate_ids = [
        (line_idx - 1, idx - 1),
        (line_idx - 1, idx),
    ...
    for candidate_id in candidate_ids:
        candidate_char = get_char_by(candidate_id[0], candidate_id[1], list_strs)
        if candidate_char.isdigit():
            yield candidate_id


def get_ids(list_strs):
...
def construct_whole_number(line_idx, idx, list_strs):
    # 0, 0, [[1,2,.],[3,4,.]] -> 12
    d = [idx]
    plus_index = idx + 1
    minus_index = idx - 1

    def is_next_digit(line_idx, idx, list_strs):
        char = get_char_by(line_idx, idx, list_strs)
        return char.isdigit()

    def is_prev_digit(line_idx, idx, list_strs):
        char = get_char_by(line_idx, idx, list_strs)
        return char.isdigit()

    while is_next_digit(line_idx, plus_index, list_strs):
        d.append(plus_index)
        plus_index += 1

    while is_prev_digit(line_idx, minus_index, list_strs):
        d.append(minus_index)
        minus_index -= 1
...
def get_number_from_id_list(line_idx, idxs, list_strs):
    # 0, [0,1], [[1,2,.],[3,4,.]] -> 12
    number = ""
    for idx in idxs:
        number += get_char_by(line_idx, idx, list_strs)
    return int(number)


def one(inpt):
    list_strs = [line for line in inpt.split("\n") if line]
    ids = list(get_ids(list_strs))
    digit_ids = []
    for line_idx, idx in ids:
        digit_ids.extend(list(digit_around_ids(line_idx, idx, list_strs)))

    line_number_idx = {}
    for line_idx, idx in digit_ids:
        if line_idx not in line_number_idx:
            line_number_idx[line_idx] = []
        line_number_idx[line_idx].append(idx)

    line_whole_number_idx = {}
    for line_idx, idxs in line_number_idx.items():
        for idx in idxs:
            if line_idx not in line_whole_number_idx:
                line_whole_number_idx[line_idx] = []
            candidate = construct_whole_number(line_idx, idx, list_strs)
            if candidate not in line_whole_number_idx[line_idx]:
                line_whole_number_idx[line_idx].append(candidate)

    number_list = []
    for line_idx, idxs in line_whole_number_idx.items():
        for idx in idxs:
            number_list.append(get_number_from_id_list(line_idx, idx, list_strs))
...
...$.*....
.664.598.."""
    one_inpt = """.............65..................998.........453...................................845..773.........................307....527...........541
