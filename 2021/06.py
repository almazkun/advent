...
class Population:
    ...
    def __repr__(self):
        return f"{len(self.fish_list)}"

    def day_passed(self):
        original_len = len(self.fish_list)
        for i in range(original_len):
            new_born = self.fish_list[i].day_passed()
...
class Sol(Solution):
    ...
    fish has an internal timer of 4, and so on until the fifth fish, which
    has an internal timer of 2. Simulating these fish over several days
