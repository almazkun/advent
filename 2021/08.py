...
class Digit:
    def __init__(self, sequence, keys=None):
    ...
    def part_two(self):
        ...
        if d is not None:
            ...
            if len(self.sequence) == 5:
                return self.two_three_five()
            elif len(self.sequence) == 6:
                return self.zero_six_nine()

    def two_three_five(self):
        four = [x for x in self.keys if len(x) == 4][0]
        seven = [x for x in self.keys if len(x) == 3][0]
        dif_four = len([x for x in self.sequence if x not in four])
        dif_seven = len([x for x in self.sequence if x not in seven])

        if dif_four == 3 and dif_seven == 3:
            return 2
        elif dif_four == 2 and dif_seven == 2:
            return 3
        elif dif_four == 2 and dif_seven == 3:
            return 5
        raise Exception(
            f"zero_two_three_five Shouldn't be here {self.sequence}, {four}, {dif}, {self.keys}"
        )

    def zero_six_nine(self):
        four = [x for x in self.keys if len(x) == 4][0]
        seven = [x for x in self.keys if len(x) == 3][0]
        dif_four = len([x for x in self.sequence if x not in four])
        dif_seven = len([x for x in self.sequence if x not in seven])
        ...
        if dif_four == 3 and dif_seven == 3:
            return 0
        elif dif_four == 3 and dif_seven == 4:
            return 6
        elif dif_four == 2 and dif_seven == 3:
            return 9
        raise Exception(
            f"zero_six_nine Shouldn't be here {self.sequence}, {four}, {dif}, {self.keys}"
        )
...
class Sol(Solution):
    ...
    [8394, 9781, 1197, 9361, 4873, 8418, 4348, 1623, 8717, 4313]
    ...
    def p2(self):
        ...
        for line in self.cleaned:
            ...
            number = []
            for digit in digits:
                digit = Digit(digit, keys)
                number.append(digit.part_two())
